...
# Desktop
setwd("C:/Users/Caleb Solomon/Documents/GitHub/School/UMD/Research/PurcellNeuroscienceOfLanguage/ntr")

AoA_measures <- read_excel("AoA_measures.xlsx", sheet = "Sheet2") # read in all AoA measures
...
vals <- c('(1,2]', '(2,3]') #add bin for every iteration of AoA
...
# Medial syllable measures are any phonemes within syllables that are NOT the beginning or end of the syllable.
...
print(length(forR_input$STRING))
for(i in 1:length(forR_input$STRING)){ 
if (i < 41){
  print(forR_input$STRING[i])
  print(i)
  print(length(syllablemedial_list)+1)
}
...
if (i < 41){
  print("first syllable medial temp")
  print(medial_temp)
}
...
medial_temp<- t(matrix(tryCatch(
  ...
  error=function(err) NA),ncol=myncol))
if (i < 41){
  print("second syllable medial temp")
  print(medial_temp)
}
...
medial_temp<- t(matrix(tryCatch(
  ...
  error=function(err) NA),ncol=myncol))
if (i < 41){
  print("third syllable medial temp")
  print(medial_temp)
}
...
medial_temp<- t(matrix(tryCatch(
  ...
  error=function(err) NA),ncol=myncol))
if (i < 41){
  print("fourth syllable medial temp")
  print(medial_temp)
}
...
medial_temp<- t(matrix(tryCatch(
  ...
  error=function(err) NA),ncol=myncol))
if (i < 41){
  print("fifth syllable medial temp")
  print(medial_temp)
}
...
medial_temp<- t(matrix(tryCatch(
  ...
  error=function(err) NA),ncol=myncol))
if (i < 41){
  print("sixth syllable medial temp")
  print(medial_temp)
}
...
for(i in 1:length(syllablemedial_list)){
  # Basically any time a word did not have any medial syllable phonemes in one of its syllables, fill that in with a 1x3 NA matrix
  if (i == 39 | i == 40){
    print(i)
    print(forR_input$STRING[i])
    print(syllablemedial_list[i])
    print(syllablemedial_list[[i]])
  }
}

# Error was in commented out section
#replace empty matrices with NAs of 1x3 so that they can be bound with the non-empty matrices
for(i in 1:length(syllablemedial_list)){
  # Basically any time a word did not have any medial syllable phonemes in one of its syllables, fill that in with a 1x3 NA matrix
  if (i == 39 | i == 40 | i == 230){
  ...
  # Trying an if instead
  if (all(is.na(syllablemedial_list[[i]]))) {
  syllablemedial_list[[i]] <- matrix(NA, nrow = 1, ncol = 3)
}
}

# Attempting to replace to ensure all is 1x3. Doesn't work because can't enforce 1:3 for a matrix that is 1:2, which is case with i=39
#for (i in 1:length(syllablemedial_list)) {
#  if (all(is.na(syllablemedial_list[[i]]))) {
#    # If all elements are NA, initialize the matrix with 3 columns
#    syllablemedial_list[[i]] <- matrix(NA, nrow = 1, ncol = 3)
#  } else {
#    # If not all elements are NA, ensure the matrix has 3 columns
#    syllablemedial_list[[i]] <- as.matrix(syllablemedial_list[[i]])
#    if (ncol(syllablemedial_list[[i]]) != 3) {
#      # If the matrix doesn't have 3 columns, expand or trim it
#      print(i)
#      print(dim(syllablemedial_list[[i]]))
#      syllablemedial_list[[i]] <- syllablemedial_list[[i]][, 1:3]
#    }
#  }
#}
